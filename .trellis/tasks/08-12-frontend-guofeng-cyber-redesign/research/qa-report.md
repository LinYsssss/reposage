...
## 2026-08-13 — Production Vue vertical slice

### Scope

- Real authentication gate and existing HttpOnly-cookie/CSRF contract.
- Ink AppShell, responsive CaseIndex, real Agent Run selector/state, Timeline, Findings/citations, Patch Diff, AnnotationRail and approval confirmation.
- Ambient Taiji/particles/cloud/mountain plane with static/reduced/coarse/hidden/unfocused degradation.
- Existing routes and API/SSE contracts remain intact; non-migrated pages continue to use their current views.

### Engineering validation

- Node `v22.23.2`; `npm ci` from `registry.npmjs.org`, 0 vulnerabilities.
- `npm test`: **22/22 passed**.
- `npm run build`: passed. Output: CSS 235.36 kB / 34.10 kB gzip; JS 544.33 kB / 188.68 kB gzip.
- `git diff --check -- frontend`: passed.
- Trellis context validation: passed.
- Build retains the pre-existing >500 kB chunk warning; route-level code splitting remains a follow-up optimization, not a functional blocker.

### Browser evidence

Location: `research/qa/production/`.

- `production-browser-final.json`: login validation, real mocked API flow, 1440/768/390 metrics, drawer inert/focus/body lock, approval Dialog focus/return, reduced motion and zero unexpected browser issues.
- `production-static-final.json`: static mode changes paper/taiji animations to `none`, resets mountain transforms and retains a low-opacity static particle field.
- Screenshots: `final-login-1440.png`, `final-agent-1440.png`, `final-agent-768.png`, `final-agent-768-rail.png`, `final-agent-390.png`, `final-agent-390-nav.png`, `final-agent-390-rail.png`, `final-agent-390-approval.png`, `final-agent-390-reduced.png`, `final-agent-390-static-verified.png`.

### Results

- No page-level horizontal overflow at 1440, 768 or 390.
- Desktop Agent layout: 236px CaseIndex + main plane; Agent content splits to 810px workspace + 292px AnnotationRail at the measured 1440 viewport.
- Tablet keeps the 218px CaseIndex and converts AnnotationRail to an inert off-canvas drawer.
- Mobile converts both rails to inert drawers; Diff uses local 320→720px horizontal scrolling; Annotation trigger is fixed in the sticky topbar at 44×44px.
- Login empty-submit semantics, authenticated project load, 3 Findings, 10 Diff rows, approval eligibility and approval confirmation all passed.
- Expected unauthenticated `/auth/me` 401 is recorded separately as the authentication probe; unexpected console/network issues: **0**.

### Readiness

- Production vertical-slice gate: **Passed**.
- Residual Critical/High in this slice: **None**.
- Remaining task scope: migrate Dashboard, Projects, Repository, PullRequests, Knowledge and AI Logs; run full real-backend Stage 7 including permission/offline/long-content/performance coverage.
