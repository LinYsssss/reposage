...
5. [x] 终验：`npm ci && npm test && npm run build`、体积对比落档、CI 绿、服务器演示动线人工走查。（干净安装 21/21+build 绿+audit 0;终态 gzip 219.24(+135.1,预算内,数字经 trellis-check 校正)落 bundle-size.md;CI 与部署走查在 trellis-check 后执行)
6. [x] `trellis-check`（Agent）→ 提交推送 → `/trellis:finish-work`。（check 于 2026-08-11 执行,咬出三处并修正:bundle 口径漏加 html 列、color-scheme meta 未随亮色定稿、两处注释与 styles.css 退役事实脱节;校正随 2026-08-12 finish-work 会话提交推送并归档）
