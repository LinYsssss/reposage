...
本项目的单一本地质量门是 `scripts/verify-local.ps1`（后端测试 + 前端测试 + 前端构建 + 模型检查 + 冒烟 + Docker 检查）；CI（`.github/workflows/ci.yml`）在 push / PR 上运行等价的 `mvn verify` + `npm ci/test/build`。
...
数据库迁移由 **Flyway** 管理（仅 prod profile 启用；开发/测试用 H2 + Hibernate，Flyway 关闭）：

- 任何生产 schema 变更都要新增不可变的 `backend/src/main/resources/db/migration/V<N>__描述.sql`。
- 已发布的迁移文件**绝不能再编辑**（Flyway 用校验和锁定历史），只能追加新版本号。
- 迁移对旧库幂等：`baseline-on-migrate` 允许非空旧库平滑升级，V2 对缺失列/索引做 `if not exists` 补齐与数据回填。
- 无 Docker 的机器上，依赖 Postgres/RabbitMQ 容器的集成测试（Testcontainers）会自动跳过。
