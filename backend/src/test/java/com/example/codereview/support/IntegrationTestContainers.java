...
/**
 * Shared PostgreSQL (pgvector) and RabbitMQ containers for integration tests.
 *
 * <p>Subclasses annotate themselves with
 * {@code @Testcontainers(disabledWithoutDocker = true)} so the whole test class is skipped
 * (not failed) on machines without Docker; CI runners with Docker execute them for real.
 */
