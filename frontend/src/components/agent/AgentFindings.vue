<template>
  <section class="panel">
    ...
    <article v-for="finding in findings" :key="finding.id" class="finding">
      ...
      <details v-if="(finding.evidence || []).length" class="evidence-drawer"><summary>展开证据与 citation</summary><ul><li v-for="e in finding.evidence || []" :key="e.contentHash || e.reference"><code>{{ e.reference || e.filePath }}</code><span>{{ e.excerpt || e.sourceVersion || '已记录 citation' }}</span><a v-if="e.filePath || e.path" :href="fileLink(e)" class="evidence-link">定位文件</a></li></ul></details>
