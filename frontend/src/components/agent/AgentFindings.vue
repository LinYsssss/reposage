<template>
  <section class="panel">
    ...
    <article v-for="finding in findings" :key="finding.id" class="finding">
      <div class="finding-head"><b><span class="badge" :class="'sev-' + finding.severity">{{ finding.severity }}</span> {{ finding.title }}</b><span class="confidence">置信度 {{ pct(finding.confidence) }}</span></div>
