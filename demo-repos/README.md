...
| 类别 | 条数 | 含义 |
|---|---|---|
| A 类 · 通用缺陷 | 4 | SQL 拼接、URL 参数未编码、原型污染这类形态特征明显的问题，静态扫描也能发现 |
...
演示时注意区分同一处代码上的两条缺陷：`search_users` 的 SQL 是 f-string 拼的（T5，纯 A 类，
本仓库 `bug-history.md` 里没有 SQL 注入事故可挂靠），第 1 档就该报出注入；但「`tenant_id` 取自
请求参数」（T1）在第 1 档不该出现。同一段代码、两种结论，这个对照比换个函数更有说服力。
