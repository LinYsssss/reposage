...
| 类别 | 条数 | 含义 |
|---|---|---|
| A 类 · 通用缺陷 | 3 | SQL 拼接、硬编码、原型污染这类形态特征明显的问题，静态扫描也能发现 |
...
| 档 | 关联文档 | 预期发现范围 |
|---|---|---|
| 第 1 档 | 不关联 | 只有 A 类。SQL 拼接、硬编码密钥、原型污染这类问题。B/C 类即便偶然提到也说不出依据 |
...
最适合单独拎出来对比的是 T1 与 T2：函数接收 `tenant_id` 参数并用它过滤，代码层面
完全「正常」，只有读过 `tenant-isolation.md` 第 2 节第 3 条才知道这是越权。这类缺陷
纯代码审查发现不了，三档之间的差异在它们身上最清楚。
